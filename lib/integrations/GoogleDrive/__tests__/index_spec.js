...
describe("GoogleDrive", () => {
    ...
    describe("credentials", () => {
      ...
      describe("connection", () => {
        ...
        describe("authenticate", () => {
          it("fails with an error when passed in no props", () => {
            expect(() => {
  ...
  describe("credentials file", () => {
    ...
    beforeEach(() => {
      ...
      integration = app.integration("GoogleDrive");
      gdrive_instance = integration({
        authentication_token: "authy_the_authonator",
        redirect_uri: "fake_redirect_uri"
      });
    });

    it("is loaded if credentials are not passed in", () => {
      expect(gdrive_instance.credentials).toEqual(expected);
    });
  });
  ...
  describe("authentication_token", () => {
    ...
    describe("fs.writeFile on error function", () => {
      ...
      it("calls callback if null", () => {
        fs.writeFile.mock.calls[0][3]();
        expect(callback).toBeCalled();
      });
    });

    describe("when there is no callback", () => {
      beforeEach(() => {
        fs = require("fs");
        fs.writeFile = jest.fn();
        callback = jest.fn();
        data_oAuth2Client_on = jest
          .fn()
